...
select 'new inventory count vs website',
...
select 'used inventory count vs website',
case when (abs(@usedActiveCount-count(l.vin))/count(l.vin))*100 between @range and @minusRange then 'fail' else 'pass' end as 'pass/fail'
from DealerSite..inventory i 
left join inventory..Listing l on l.DealerID = i.DealerID and l.vin = i.vin
where i.dealerid = @dealerID
and i.InventoryStatusId = 1
