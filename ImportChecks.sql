...
case when @newTotalCount = 0 then 'no new inventory' when count(i.vin) = 0 then 'fail' else case when (abs(@newActiveCount-count(l.vin))/count(l.vin))*100 between @range and @minusRange then 'fail' else 'pass' end end as 'pass/fail'
from DealerSite..inventory i 
...
case when ((abs(count(i.vin) - @newTotalCount)) / @newTotalCount) * 100 between @range and @minusRange then 'fail' else 'pass' end end as 'pass/fail'
from DealerSite..inventory i
left join inventory..Listing l on l.dealerid = i.dealerid and l.vin = i.vin
...
and l.ListingStatusID = 1
and i.listingtypeid = 1
and ((isnull(i.price, 0.00) - isnull(ld.price, 0.00)) / isnull(ld.price, 0.00))*100 > @range
...
case when @newTotalCount = 0 then 'no new inventory' else
case when ((abs(count(i.vin) - @newTotalCount)) / @newTotalCount) * 100 < @range then 'fail' else 'pass' end end as 'pass/fail'
from DealerSite..inventory i
left join DealerSite..inventoryphoto ip on i.InventoryID = ip.InventoryID and i.dealerid = ip.DealerID
where i.dealerid = @dealerid
and i.ListingTypeID = 1
...
case when count(l.vin) != 0 then
case when (abs(@usedActiveCount-count(l.vin))/count(l.vin))*100 between @range and @minusRange then 'fail' else 'pass' end 
else 'no used crawling' end
else 'no used inventory' end as 'pass/fail'
from DealerSite..inventory i 
left join inventory..Listing l on l.DealerID = i.DealerID and l.vin = i.vin
where i.dealerid = @dealerID
and i.InventoryStatusId = 1
...
case when @usedTotalCount = 0 then 'no used inventory' else
case when ((abs(count(i.vin) - @usedTotalCount)) / @usedTotalCount) * 100 between @range and @minusRange then 'fail' else 'pass' end end as 'pass/fail'
from DealerSite..inventory i
left join inventory..Listing l on l.dealerid = i.dealerid and l.vin = i.vin
left join inventory..ListingDetail ld on ld.listingid = i.ListingID
where i.dealerid = @dealerid
...
case when @usedTotalCount = 0 then 'no used inventory' else
case when ((abs(count(i.vin) - @usedTotalCount)) / @usedTotalCount) * 100 < @range then 'fail' else 'pass' end end as 'pass/fail'
from DealerSite..inventory i
left join DealerSite..inventoryphoto ip on i.InventoryID = ip.InventoryID and i.dealerid = ip.DealerID
where i.dealerid = @dealerid
and i.ListingTypeID = 2
